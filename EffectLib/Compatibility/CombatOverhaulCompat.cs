...
#pragma warning restore IDE0130 // Namespace does not match folder structure
{
    public static class CombatOverhaulCompat
    {
        ...
        private const string CompatAssemblyName = "EffectLibCombatOverhaulCompat";
        private const string CompatEntryTypeName =
            "EffectLib.CombatOverhaulCompatBridge.CompatEntry";
        ...
        public static void Init(ICoreAPI api)
        {
            ...
            if (buffSystemMs == null)
            {
                api.Logger.Notification(
                    "[EffectLib] Combat Overhaul weapon buff system not found; weapon coatings "
            ...
            try
            {
                ...
                Active = true;
                api.Logger.Notification(
                    "[EffectLib] Combat Overhaul weapon coating compatibility enabled"
                );
            }
            catch (Exception e)
            {
                ...
                api.Logger.Warning(
                    "[EffectLib] Could not enable Combat Overhaul weapon coating compatibility, "
        ...
        private static Assembly LoadEmbeddedCompatAssembly()
        {
            ...
            if (resourceName == null)
                throw new InvalidOperationException(
                    $"Embedded resource {CompatAssemblyName}.dll not found in effectlib.dll"
                );
        ...
        private static void BindCompatEntry(ICoreAPI api)
        {
            ...
            // Hooks passed as delegate args (rather than the compat assembly calling
            // CoatingPolicy/CoatedEffects/EffectLang directly) so EffectLibCombatOverhaulCompat
            // never needs a compile-time reference back to EffectLib.csproj - EffectLib.csproj
            // already references it the other way (to embed its built dll as a resource), and a
            // reference in both directions is a circular project dependency MSBuild can't build.
            entry.GetMethod("Init")
                .Invoke(
                    null,
                    [
                        api,
